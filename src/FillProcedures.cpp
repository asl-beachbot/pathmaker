...
public:
  ...
  void run() {
    ...
    int idx_closest = 0; // TODO set to some reasonable value
    ...
    while(offset_poly_wh.size() > 0) {
      ...
      if(superline.size())
        idx_closest = find_closest_index(superline.back(), outer);

      superline.push_back(outer[(idx_closest + 1) % outer.size()]);
      idx += 1;
...
private:
  int find_closest_index(Point_2 p, Polygon_2 poly) {
    double temp_dist = 9999999;
    double distance = 9999999;
    int ret_idx = 0;
    for(int i = 0; i < poly.size(); ++i) {
      temp_dist = CGAL::squared_distance(p, poly[i]);
