...
std::vector<BezierCP> ConnectionSmoother::getBezierCPS(
  ...
  // Another idea: Angles > 90 degree limit to 90 degree ?


  if (conn_angle_1 < M_PI/4 && conn_angle_2 < M_PI/4 && vec_angle < M_PI/4) {
    no_curve_points = true;
  }
  else {

    // if curve points are on same side
    // and have big angles, then curve will extend to infinity

    auto side1 = -d1[1] * conn_line_n[0] + d1[0] * conn_line_n[1];
    auto side2 = -d2[1] * conn_line_n[0] + d2[0] * conn_line_n[1];

    cout << "Sides: " << side1 << " 2: " << side2 << endl;
    cout << sgn(side1) << " S2: " << sgn(side2) << endl;
    ...
    if(
      ...
      cout << "both points on same side!" << endl;

      int pre1 = -sgn(side1);
      int pre2 = sgn(side2);
      auto r1 = Transformation(CGAL::ROTATION, sin( pre1 * conn_angle_1/2), cos(pre1 * conn_angle_1/2));
      auto r2 = Transformation(CGAL::ROTATION, sin( pre2 * conn_angle_2/2), cos(pre2 * conn_angle_2/2));

      pm11 = pm1 + d1.transform(r1) * ( rr);
      pm22 = pm2 + d2.transform(r2) * ( rr);
