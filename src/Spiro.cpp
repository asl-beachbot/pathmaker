...
void
Spiro::compute_pderivs(const spiro_seg *s, double ends[2][4], double derivs[4][2][4],
		int jinc)
{
		double recip_d = 2e6;
...
int Spiro::compute_jinc(char ty0, char ty1)
{
		if (ty0 == 'o' || ty1 == 'o' ||
	ty0 == ']' || ty1 == '[')
	return 4;
...
int Spiro::count_vec(const spiro_seg *s, int nseg)
{
		int i;
		int n = 0;
...
int
Spiro::solve_spiro(spiro_seg *s, int nseg)
{
		bandmat *m;
		double *v;
		int *perm;
